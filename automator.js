...
for (let i in Game.Upgrades) { 
  ...
  if (Game.Upgrades[i].pool == "toggle") { 

    blacklist.push(Game.Upgrades[i].name); 
...
let loopDelay = 50;  // Initial delay in ms 

function runLoop() { 

    if (!isRunning) return; // Stop if the loop is not running 
    ...
    for (let i in CookieMonsterData.Upgrades) { 

        if (CookieMonsterData.Upgrades[i].colour == "Blue" && !blacklist.includes(i)) { 

            // Check if the upgrade is not in the blacklist 
    ...
    if (flag) { 
        ...
        loopDelay = 50;  // Reset delay after action 

        requestAnimationFrame(runLoop);  // Use requestAnimationFrame for smoother loop 
    ...
    for (let i in CookieMonsterData.Objects1) { 

        if (CookieMonsterData.Objects1[i].colour == "Green") { 

            Game.Objects[i].buy(1); 
    ...
    for (let i in CookieMonsterData.Objects10) { 

        if (CookieMonsterData.Objects10[i].colour == "Green") { 

            Game.Objects[i].buy(10); 
    ...
    for (let i in CookieMonsterData.Objects100) { 

        if (CookieMonsterData.Objects100[i].colour == "Green") { 
    ...
    // If any operation was performed, continue with a short delay 
    ...
    // If no action was performed, add a longer delay before next iteration 

    loopDelay = Math.min(loopDelay * 1.5, 200);  // Increase delay gradually, cap at 200ms 

    setTimeout(() => requestAnimationFrame(runLoop), loopDelay);  // Use requestAnimationFrame with delay 
...
var autoNormieWrinklerKiller = setInterval(function() {for (var i = 0; i < 14; i++) {if(Game.wrinklers[i].type==0 && Game.wrinklers[i].sucked>0){Game.wrinklers[i].hp=0}}},1000); 

Game.LoadMod("https://glander.club/asjs/8qsFdMRn"); 

!function() { Game.registerMod("autosacrifice", { auto_sac_enabled: false, do_convert: false, avoid_cps_buffs: false, allow_frenzies: false, do_soil_rotation: false, 
